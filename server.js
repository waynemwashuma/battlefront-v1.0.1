...
const fileStore = require('session-file-store')(sessionHandler)
...
var SESS_PATH = __dirname + '/php/tmp/';
let clients = new Map();
...
app.post('/login', function (req, res) {
    ...
    conn.query("SELECT * FROM users WHERE userName = ? OR userMail = ? ", [
        ...
        if (!err) {
            ...
            res.cookie('sessId',req.sessionID,{maxAge:1000*60});
...
app.post('/leaveAlliance', (req, res) => {
    ...
    conn.query('SELECT * alliances WHERE ?'), [req.body.alliname], (err, results) => {
        if (!err) {
            if (!results[0].members.includes(req.session.name)) return res.send('You are not in this alliance');
            members = results[0].members.split(';') || [];
            for (let i = 0; i < members.length; i++) {
                count += members[i].length;
                q = members[i].split('=');
                if (q[0] == 'leader') return res.send('You cannot leave alliance as you are the leader')
                if (q[1] !== req.session.name) continue;
                results[0].members.slice(count - 1, q[0].length + q[1].length + count);
                conn.query("UPDATE alliances SET members = ? WHERE ?", [
                    results[0].members, req.body.alliname
                ])
            }
...
app.post('/joinAlliance', function (req, res) {
    let members, processedMembers = [], q;
    if (!req.cookies.sessId) return res.status(500).send('session ended:Please login and try again');
    if (!req.body.alliance) return res.send('please select a valid alliance');
    conn.query('SELECT * alliances WHERE ?', [req.body.alliance], (err, results) => {
        if (!err) {
            ...
            results[0].members = results[0].members.concat(`member=${req.session.name}`)
            conn.query('UPDATE SET members = ? WHERE  ?',[results[0].members,req.body.alliname])
        }
    })
});
...
server.use((socket, next) => {
    ...
    if (cookie.parse(socket.handshake.headers.cookie).sessId === undefined) {
...
server.on('connection', socket => {
    ...
    if (typeof socket.handshake.headers.cookie === "string") {
        ...
        if (typeof sid.sessId === "undefined") {
            ...
            fs.readFile(__dirname + "/sessions/" + sid.sessId + '.json', 'utf-8', function (err, data) {
                if (!err) {
                    sd = JSON.parse(data);
                    clients.forEach(c => {
